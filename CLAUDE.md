...
A labor **contraction timer** web app. The app — markup, styles, and logic — lives in [index.html](index.html). There is **no build step, no dependencies to install, no tests, and no package.json**. Three sibling files add PWA/offline support: [sw.js](sw.js) (service worker), [manifest.webmanifest](manifest.webmanifest), and [logo.png](logo.png) (app icon, 1024×1024).
...
External resources are loaded from CDNs at runtime (Tailwind Play CDN, Iconoir icon font, Inter via Google Fonts). This means the app needs network on first load and the Tailwind CDN prints a "not for production" console warning by design.
...
- **Tailwind is the only styling mechanism** (utility classes inline in the templates). The Play CDN's MutationObserver picks up classes Preact renders dynamically (e.g. the green `bg-emerald-500` "met" bar, `ring-…` intensity selection, `LEVELS[...].fill` colors), so they work even though they never appear statically. What utilities can't express lives in the `<style>` block: the `.picker` styled-select, the `accent-color` rose tint on native inputs, the `row-in` keyframe, and `tabular-nums` on `body`.

- **Theming uses semantic design tokens, not `dark:` variants.** Dark mode is automatic (follows the OS via `prefers-color-scheme`). The palette is defined once as CSS variables in the `<style>` block (`--c-base`, `--c-card`, `--c-surface`, `--c-line`, `--c-track`, `--c-ink`, `--c-body`, `--c-muted`, `--c-faint`) — light values in `:root`, dark values in the `@media (prefers-color-scheme: dark)` block — and registered as Tailwind colors in the config as `rgb(var(--c-…) / <alpha-value>)`. So use the **token utilities** everywhere — `bg-base`/`bg-card`/`bg-surface`, `border-line`, `bg-track`, `text-ink`/`text-body`/`text-muted`/`text-faint` — and the theme flips for free; **do not add `dark:` variants or raw `neutral-*` colors.** The `.picker` reads the same vars. Two more named colors in the config: **`brand-*`** (the rose-pink brand scale — Start button, 5-1-1 alert, focus rings, `accent-color`) and **`danger`** (a dark-aware token, `--c-danger`, used for the destructive "Clear all" row via `text-danger` / `border-danger/40` / `hover:bg-danger/10`). Other raw colors kept on purpose: `LEVELS` intensity colors (amber/orange/red) and the modal scrim `bg-neutral-900/30`.

- **Light/dark works in three layers.** (1) Default follows the OS via `@media (prefers-color-scheme: dark)`, but scoped to `:root:not([data-theme="light"])` so a manual choice wins. (2) A manual toggle sets `data-theme="light"|"dark"` on `<html>`; `:root[data-theme="dark"]` re-declares the dark vars. (3) An **inline `<head>` script** applies the saved (`localStorage` key `contraction-timer.theme`) or system theme to `<html>` *before paint* to avoid a flash — keep it inline and early. The header toggle button (`App.toggleTheme`) flips the attribute, persists it, swaps its sun/moon icon, and updates the `theme-color` meta. `App.state.theme` mirrors the current value for the icon.
